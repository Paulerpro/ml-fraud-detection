# 📌 **README.md — Credit Card Fraud Detection (PyTorch + Streamlit + FastAPI)**

## 🧠 Overview

This project is an **End-to-End Credit Card Fraud Detection System** built using:

* **PyTorch** — for model training
* **Scikit-Learn** — for preprocessing & evaluation
* **Streamlit** — for the interactive UI
* **FastAPI (upcoming)** — for production-grade API deployment
* **MLflow (planned)** — for experiment tracking
* **Google Colab** — for GPU-accelerated training

The model is trained on the **Kaggle Credit Card Fraud Dataset**, a highly imbalanced dataset of >284,000 transactions.

This repository contains a clean, modular, production-ready Python project structure separating:

✔ Training
✔ Inference
✔ API layer
✔ UI layer
✔ Notebooks
✔ Model artifacts (`model.pt`, `scaler.pkl`)

---

# 📂 **Project Structure**

```
ml-fraud-detection/
│
├── ml_fraud_detection/
│   ├── training/
│   │   ├── train.py          # Training pipeline (from preprocessing → saving model)
│   │   ├── model.py          # MLP model architecture
│   │   ├── dataset.py        # Custom PyTorch Dataset and DataLoaders
│   │   └── utils.py          # Helper utilities (metrics, seeding, loaders)
│   │
│   ├── inference/
│   │   └── predict.py        # Loads model + scaler → predicts fraud
│   │
│   └── __init__.py
│
├── models/
│   ├── model.pt              # Exported PyTorch model weights
│   ├── scaler.pkl            # StandardScaler used during preprocessing
│
├── app/
│   ├── streamlit_app.py      # UI for interactive predictions
│
├── notebooks/
│   └── training_notebook.ipynb   # Colab training/EDA notebook
│
├── requirements.txt          # Auto-generated by uv / pip
└── README.md                 # You are here
```

---

# 🗂️ Dataset — Credit Card Fraud (Kaggle)

* **284,807 transactions**
* **492 fraud cases (0.17%)**
* Contains PCA-transformed features: `V1` → `V28`
* Additional features: `Time`, `Amount`
* Target label: `Class`

  * `0` = legitimate
  * `1` = fraud

---

# 🧪 **Model Architecture (MLP)**

A simple but effective neural network:

* Input Layer → 128 → 64 → Output
* BatchNorm
* ReLU activation
* Dropout for regularization
* Uses `BCEWithLogitsLoss` + **class weights** to handle imbalance
* Adam optimizer

---

# 🔁 Training Pipeline

The training pipeline in `train.py` handles:

### ✔ Reproducibility (seed_everything)

### ✔ Preprocessing

* Standardizes `Time` and `Amount`
* Keeps PCA components as-is

### ✔ Stratified Splits

* Train
* Validation
* Test

### ✔ Weighted BCE Loss

To counter the massive class imbalance.

### ✔ Early Stopping (AUC-based)

### ✔ Saving Artifacts

```
models/
│── model.pt
│── scaler.pkl
```

---

# 💡 Training — Run Locally

Activate your venv and install dependencies:

```bash
uv init  # creates pyproject.toml
uv add torch scikit-learn pandas numpy matplotlib seaborn fastapi uvicorn streamlit
```

Run training:

```bash
python ml_fraud_detection/training/train.py
```

---

# 🎯 **Inference (Prediction Pipeline)**

`predict.py` loads:

* `model.pt`
* `scaler.pkl`

And exposes a function:

```python
predict_single(transaction_dict)
```

Returns:

* Probability of fraud
* Binary decision

Run manually:

```bash
python ml_fraud_detection/inference/predict.py
```

---

# 🖥️ **Streamlit Application**

Interactive fraud prediction UI.

Run:

```bash
streamlit run app/streamlit_app.py
```

The UI allows users to:

* Manually input transaction data
* Get prediction probability
* Model explanation (coming soon via SHAP)

---

# ⚡ **FastAPI Integration (Next Step)**

The backend API (`app/fastapi_app.py`) will expose:

### **POST /predict**

Request:

```json
{
  "Time": 120.0,
  "Amount": 59.99,
  "V1": -1.23,
  "V2": 0.12,
  ...
  "V28": 0.87
}
```

Response:

```json
{
  "fraud_probability": 0.824,
  "is_fraud": true
}
```

### **Run FastAPI**

```bash
uvicorn app.fastapi_app:app --reload
```

This allows:

* Integrating your model with mobile apps
* Deploying behind Nginx
* Building a microservice architecture
* Streamlit frontend calling FastAPI backend

---

# 🚀 Deployment (Planned)

Upcoming deployment options:

* **Docker container**
* **Railway / Render / Azure / AWS EC2**
* **MLflow model registry**
* **FastAPI reverse proxy**

---

# 📊 Model Performance

| Metric                 | Score      |
| ---------------------- | ---------- |
| Test AUC               | **0.9660** |
| Test Average Precision | **0.722**  |

Very strong considering the extreme imbalance.

---

# 📓 **Notebook**

`training_notebook.ipynb` includes:

* EDA
* Data visualizations
* Model experimentation
* Performance plots
* Quick prototyping

Production code lives in `ml_fraud_detection/`.

---

# 🧰 **Environment Setup (uv)**

Check installed dependencies:

```bash
uv pip list
```

Install new dependencies:

```bash
uv add fastapi uvicorn
```

Sync environment:

```bash
uv sync
```

---

# 👤 **Author**

**Paulerpro**

* Data Scientist / ML Engineer
* Focused on practical end-to-end AI systems
* Passion for med-tech, ML engineering, and automation

---

# 💬 Support

Issues? Open one in GitHub or message me.